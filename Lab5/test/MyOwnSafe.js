...
describe("MyOwnSafe", function () {
    async function deployFixture() {
        ...
        const SafeContract = await ethers.getContractFactory("contracts/MyOwnSafe.sol:MyOwnSafe"); //deploy safe contract
        const safeContract = await SafeContract.deploy(owner.address);
    ...
    // Test#2: check if owner deployed
    ...
    // Test#3: check if withdraws of tokens are correct
    ...
    // Test#4: check if count only calls if owner does it
    ...
    // Test#5: check if count function works as expected
