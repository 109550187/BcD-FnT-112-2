...
def gen_merkle_proof(leaves, pos):
    ...
    for level in range(height):
        ...
        # start by appending path from the current leaf.
        if level_pos % 2 == 0:
            path.append(state[level_pos + 1]) #append right leaf if level_pos is even
        else:
            path.append(state[level_pos - 1]) #append left leaf if level_pos is odd

        # change level_pos to parent/branch node after appending.
        level_pos //= 2

        # change the state of hashed leaves into current parent/branch node level
        branch = []
        for left, right in zip(state[::2], state[1::2]):
            # using the zip function to iterate over pairs of sibling nodes,
            # store the new hashed leaves into a branch list
            branch.append(hash_internal_node(left, right))
        # update the state list into the newer branch list.
        state = branch
